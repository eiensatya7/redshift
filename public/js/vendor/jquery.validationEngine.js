    ...
    var methods = {
        ...
        attach: function(userOptions) {
            ...
            if (!options.binded) {
					...
					} else if (options.bindMethod == "delegate") {
                        // bind fields with delegate (for persistant state)
                        form.find("[class*=validate]").not("[type=checkbox]").not(".datepicker").delegate(options.validationEventTrigger, methods._onFieldEvent);
                        form.find("[class*=validate][type=checkbox]").delegate("click", methods._onFieldEvent);

						form.find("[class*=validate][class*=datepicker]").delegate(options.validationEventTrigger,{"delay": 300}, methods._onFieldEvent);
